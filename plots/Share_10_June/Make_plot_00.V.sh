...
cat ../Select_COVID_data_PEAKS.UNSM.ALL._00.ftxt | egrep "Country|$1 " | best_fit_line.newV.exe > $1._00.log
...
rm input.gp ; ln -s $1_Confirmed._00.gp input.gp     ; rm input.tab ; ln -s $1_Confirmed._00.tab input.tab; rm input.bes ; ln -s $1_Confirmed._00.best input.bes
cat Generic_BLP_plot.gnu | sed "s/LOCATION/$1/" | sed "s/TYPE/Confirmed/" | sed "s/DATE/$date/" | sed "s/VERSION/_00/" | gnuplot 

rm input.gp ; ln -s $1_Deaths._00.gp input.gp        ; rm input.tab ; ln -s $1_Deaths._00.tab input.tab;    rm input.bes ; ln -s $1_Deaths._00.best input.bes   
